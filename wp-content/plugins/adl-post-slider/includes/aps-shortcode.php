 ...
 *
 * @param string $read_more_text "Read more" excerpt string.
 * @return string (Maybe) modified "read more" excerpt string.
 */
function aps_excerpt_more( $read_more_text )
{
    if (!is_admin()) {
 ...
 * @param int $read_more_text
 *
 * @return array|mixed|string
 */
function aps_get_limited_excerpts($limit = 50,$read_more_text) {
    $excerpt = explode(' ', get_the_excerpt(), $limit);
    if (count($excerpt)>=$limit) {
        array_pop($excerpt);
        $moreText = apply_filters('excerpt_more', $read_more_text);
...
// temp vars
    ...
    $read_more_text = get_post_meta( $shorcode_id, 'read_more_text', true );
